...
mapa_estadistico <- function(
        ...
        if (!is.null(comunas_requeridas)) {
                ...
                df_user <- df_user %>% dplyr::filter(nombre_comuna %in% comunas_requeridas)
        }
        ...
        mapa_join <- mapa_sf %>%
                dplyr::left_join(df_user, by = "nombre_comuna")
        ...
        if (tipo == "discreto") {
                mapa_join <- mapa_join %>%
                        dplyr::mutate(
                                grupo = dplyr::ntile(-valor, 3),
                                grupo = factor(grupo, labels = c("Superior", "Medio", "Inferior")),
                                fill_var = grupo
                        )
        } else {
                mapa_join <- mapa_join %>%
                        dplyr::mutate(fill_var = valor)
        }
        ...
        # gráfico
        g <- ggplot2::ggplot(mapa_join) +
             ggplot2::geom_sf(ggplot2::aes(geometry = geometry, fill = fill_var)) +
             ggplot2::geom_sf_text(ggplot2::aes(label = nombre_comuna, text = text_label), size = 3.1, fontface = "bold", color = "black") +
             ggplot2::labs(title = titulo, fill = titulo_leyenda, x = "Longitud", y = "Latitud") +
             ggplot2::theme_light(base_size = 10) +
             ggplot2::theme(legend.position = "bottom", legend.key.size = grid::unit(1, "cm"))

        if (tipo == "discreto") {
                g <- g + ggplot2::scale_fill_manual(values = rev(colores))
        } else {
                g <- g + ggplot2::scale_fill_gradientn(colours = colores)
        }
